...
/**
 * \file    importbatch/admin/setup.php
 * \ingroup importbatch
 * \brief   ImportBatch setup page.
 */
...
require_once '../lib/importbatch.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "importbatch@importbatch"));
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/importbatch/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'IMPORTBATCH_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'IMPORTBATCH_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$head = importbatchAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', $langs->trans($page_name), -1, "importbatch@importbatch");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("ImportBatchSetupPage").'</span><br><br>';
...
if ($action == 'edit') {
	...
	print '<tr class="liste_titre"><td class="titlefield">'.$langs->trans("importbatch").'</td><td>'.$langs->trans("Value").'</td></tr>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'IMPORTBATCH_'.strtoupper($myTmpObjectKey).'_ADDON';
