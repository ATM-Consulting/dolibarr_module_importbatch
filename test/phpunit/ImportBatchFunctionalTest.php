...
/**
 * \file    test/phpunit/ImportBatchFunctionalTest.php
 * \ingroup importbatch
...
/**
 * Class ImportBatchFunctionalTest
 *
 ...
 *
 * @package Testimportbatch
 */
class ImportBatchFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modImportBatch')->click();
		} else {
			// Disable the module
			$this->byHref('modImportBatch')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/importbatch/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('importbatch/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/importbatch/admin/about.php');
		$this->authenticate();
		return $this->assertContains('importbatch/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('importbatchwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modImportBatch_ImportBatchTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modImportBatch_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
